...
@Component
public class CountMessageHandler implements MessageHandler {

    private final ChatCache cache;

    public CountMessageHandler(ChatCache cache) {
        this.cache = cache;
    }

    @Override
    public BotState state() {
        return BotState.ADD_COUNT_PARAMETER;
    }

    @Override
    public List<PartialBotApiMethod<Message>> handleMessage(Message message) {
        cache.saveChatOperationState(message.getChatId(), OperationState.COUNT_PARAMETER);
        cache.setChatBotState(message.getChatId(), BotState.PROCESS_MESSAGE_PARAMETERS);

        String text = "Введите количество фильмов при поиске, например:\n10";
