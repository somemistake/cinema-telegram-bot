...
@Component
public class CountryMessageHandler implements MessageHandler {

    private final ChatCache cache;

    public CountryMessageHandler(ChatCache cache) {
        this.cache = cache;
    }

    @Override
    public BotState state() {
        return BotState.ADD_COUNTRY_PARAMETER;
    }

    @Override
    public List<PartialBotApiMethod<Message>> handleMessage(Message message) {
        cache.saveChatOperationState(message.getChatId(), OperationState.COUNTRY_PARAMETER);
        cache.setChatBotState(message.getChatId(), BotState.PROCESS_MESSAGE_PARAMETERS);

        String text = "Введите страну(ы), например:\nЯпония\nЯпония, Россия, ...";
